...
const DEADLINE_LABEL: Record<"ok" | "warn" | "critical", string> = {
  ok:       "успевает",
  warn:     "мало времени",
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap items-start gap-2 p-2.5 rounded-md border border-[oklch(0.52_0.22_25/0.35)] bg-[oklch(0.52_0.22_25/0.08)]">
        <div className="flex-1 min-w-0">
          <p className="text-[11px] text-[oklch(0.72_0.18_25)] font-medium leading-relaxed">
            {isPlanned ? (
              <>
                Плановый объект <span className="font-mono">{objectName}</span> ещё не введён в добычу.
                Необходимо оформить {items.length} пакет{items.length > 1 ? "а/ов" : ""} документов{" "}
                <span className="font-semibold">до запуска</span>.
              </>
            ) : (
              <>
          ...
          <p className="text-[10px] text-muted-foreground mt-0.5 font-mono">
            ...
            {plannedLaunchDate && (
              <> · плановый запуск: <span className="font-semibold">{plannedLaunchDate}</span></>
            )}
          </p>
          {isPlanned && (criticalCount > 0 || warnCount > 0) && (
            <p className="text-[10px] text-[oklch(0.72_0.18_25)] mt-1 font-medium">
              {criticalCount > 0 && (
                <span>Критично: {criticalCount} модул{criticalCount > 1 ? "я" : "ь"} не успевает оформить до запуска. </span>
              )}
              {warnCount > 0 && (
                <span>Предупреждение: {warnCount} модул{warnCount > 1 ? "я" : "ь"} — менее 30 р.д. до запуска.</span>
              )}
            </p>
          )}
        </div>
      ...
      {items.map((item) => {
        ...
        return (
          <div key={item.module} className="rounded-md border border-border bg-card overflow-hidden">
            {/* Header */}
            <button
                ...
                {item.plannedDeadline && ds && (
                  <div className={`flex items-center gap-2 px-3 py-2 border-b border-border/50 ${DEADLINE_STYLES[ds]}`}>
                    ...
                    <span className="text-[10px] font-medium">
                      Крайний срок оформления:{" "}
                      <span className="font-mono font-bold">{item.plannedDeadline}</span>
